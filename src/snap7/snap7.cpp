...
int TS7Client::S7_get_int_from_buffer(byte Buffer[], int Pos) {
    int16_t  ret_val;
    uint16_t res;
    res = Buffer[Pos]; res <<= 8;
    res |= Buffer[Pos + 1];
    memcpy (&ret_val, &res, 2);
    return static_cast<int>(ret_val);
}

bool TS7Client::S7_get_bool_from_buffer(byte Buffer[], int Pos, int Bit)
{
...
uint8_t TS7Client::S7_get_byte_from_buffer(byte Buffer[], int Pos) {
    return Buffer[Pos];
}

int TS7Client::S7_get_dint_from_buffer(byte Buffer[], int Pos) {
    uint32_t res;
    res = Buffer[Pos]; res <<= 8;
    res |= Buffer[Pos + 1]; res <<= 8;
    res |= Buffer[Pos + 2]; res <<= 8;
    res |= Buffer[Pos + 3];

    int32_t  ret_val;
    memcpy (&ret_val, &res, 4);
    return static_cast<int>(ret_val);
}

uint16_t TS7Client::S7_get_word_from_buffer(byte Buffer[], int Pos) {
    uint16_t res;
    res = Buffer[Pos]; res <<= 8;
    res |= Buffer[Pos + 1];
...
double TS7Client::S7_get_real_from_buffer(byte Buffer[], int Pos) {
...
uint32_t TS7Client::S7_get_dword_from_buffer(byte Buffer[], int Pos) {
...
void TS7Client::S7_make_real_to_buffer(byte Buffer[], int Pos, float Value) {
    uint32_t Pack;
    memcpy (&Pack, &Value, 4);
    Buffer[Pos + 3] = (byte)(Pack & 0xFF);
    Buffer[Pos + 2] = (byte)((Pack >> 8) & 0xFF);
    Buffer[Pos + 1] = (byte)((Pack >> 16) & 0xFF);
    Buffer[Pos] = (byte)((Pack >> 24) & 0xFF);
}

void TS7Client::S7_make_int_to_buffer(byte Buffer[], int Pos, int16_t Value) {
    uint16_t Pack;
    memcpy (&Pack, &Value, 2);
    Buffer[Pos + 1] = (byte)(Pack & 0xFF);
    Buffer[Pos] = (byte)((Pack >> 8) & 0xFF);
}

void TS7Client::S7_make_dint_to_buffer(byte Buffer[], int Pos, int32_t Value) {
    uint32_t Pack;
    memcpy (&Pack, &Value, 4);
    Buffer[Pos + 3] = (byte)(Pack & 0xFF);
    Buffer[Pos + 2] = (byte)((Pack >> 8) & 0xFF);
...
void TS7Client::S7_make_word_to_buffer(byte Buffer[], int Pos, uint16_t Value) {
    uint16_t Pack;
    memcpy (&Pack, &Value, 2);
    Buffer[Pos + 1] = (byte)(Pack & 0xFF);
    Buffer[Pos] = (byte)((Pack >> 8) & 0xFF);
...
void TS7Client::S7_make_dword_to_buffer(byte Buffer[], int Pos, uint32_t Value) {
    uint32_t Pack;
    memcpy (&Pack, &Value, 4);
    Buffer[Pos + 3] = (byte)(Pack & 0xFF);
    Buffer[Pos + 2] = (byte)((Pack >> 8) & 0xFF);
